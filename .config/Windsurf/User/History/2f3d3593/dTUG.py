...
base_rgba = original.convert("RGBA")
source_color = guess_color(base_rgba)

# Build a *fresh* smooth rotation from the first frame only
NUM_FRAMES = 90  # 4° per frame gives 90 frames for full circle
ANGLE_STEP = 360 / NUM_FRAMES

base_coloured = replace_color_fuzzy(base_rgba.copy(), source_color, gruvbox_yellow)
recolored_frames = [
    base_coloured.rotate(i * ANGLE_STEP,
                         resample=Image.BICUBIC,
                         expand=False,
                         center=(base_coloured.width // 2, base_coloured.height // 2))
