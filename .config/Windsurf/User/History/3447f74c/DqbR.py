...
sensitivity_factor = 3.0  # BOOSTED for more natural feel
acceleration_gain = 2.0   # matches Raw Accel Cap
acceleration_factor = 14.0  # sharper rise, closer to Raw Accel

# Device name as seen in `hyprctl devices`
# e.g. 'razer-razer-deathadder-essential'
if len(sys.argv) < 2:
...
# Sigmoid shaped gain function
def sigmoid(x, a=acceleration_factor):
    return 1 / (1 + math.exp(-a * (x - 0.2)))

def float16x16(num):
    return struct.unpack('<i', num[:-4])[0] / int(0xffff)

# Windows Raw Accel style points (same base from earlier script)
X = [
...
def interpolate(x):
    ...
    return min(y, acceleration_gain)
